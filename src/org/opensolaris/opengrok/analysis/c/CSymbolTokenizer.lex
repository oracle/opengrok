...
%{
  ...
  public static void main(String argv[]) {
    if (argv.length == 0) {
      System.out.println("Usage : java CSymbolTokenizer <inputfiles>");
    }
    else {
      Date start = new Date();
      for (String arg: argv) {
        CSymbolTokenizer scanner = null;
        try {
          scanner = new CSymbolTokenizer( new BufferedReader(new java.io.FileReader(arg)));
	  Token t;
